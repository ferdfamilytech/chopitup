    ...
    <div className="auth screen-enter">
      ...
      <div className="auth-card">
        ...
        <div style={{marginTop:20,padding:"12px",background:"rgba(201,168,76,.05)",border:"1px dashed rgba(201,168,76,.2)",borderRadius:10,textAlign:"center"}}>
          ...
          <button onClick={()=>onAuth("barber","demo")} style={{background:"none",border:"none",color:C.gold,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Enter as Demo Barber →</button>
        </div>
      </div>
    </div>
  );
...
// ══════════════════════════════════════════
function ClientsScreen({isDemo=false}){
  const [search,setSearch]=useState("");
  const [sel,setSel]=useState(null);
  const [histTab,setHistTab]=useState("history");
  const [noteText,setNoteText]=useState("");
  const [loading,setLoading]=useState(true);
  const [clients,setClients]=useState(isDemo?CLIENTS:[]);
  ...
  return(
    <div className="screen screen-enter">
      ...
      <div className="sec" style={{marginBottom:20}}>
        <p className="stit">{loading?"Loading...":filtered.length===0&&clients.length===0?"No clients yet":`${filtered.length} clients`}</p>
        {loading?(
          <>{[1,2,3].map(i=><SkeletonCard key={i}/>)}</>
        ):clients.length===0?(
          <div style={{textAlign:"center",padding:"40px 20px",color:C.muted}}>
            <p style={{fontSize:40,marginBottom:12}}>👥</p>
            <p style={{fontSize:16,fontWeight:600,color:C.text,marginBottom:6}}>No clients yet</p>
            <p style={{fontSize:13}}>Clients will appear here after their first appointment</p>
          </div>
        ):(
          filtered.map(c=>(
...
// DASHBOARD
// ══════════════════════════════════════════
function Dashboard({onAdd,onAdmin,onBookingPage,onWaitlist,onNoShow,shopName="Fresh Cutz",isDemo=false}){
  const [online,setOnline]=useState(true);
  const mockTXS=isDemo?TXS:[];
  const total=mockTXS.filter(t=>t.date==="Today"&&t.type==="in").reduce((s,t)=>s+t.amount+t.tip,0);
  return(
    <div className="screen screen-enter">
      <div className="hdr">
      ...
      <div className="sec">
        ...
        {(isDemo?INIT_APPTS:[]).slice(0,2).map(a=>(
...
// ══════════════════════════════════════════
function Appointments({onAdd,onWaitlist,onNoShow,isDemo=false}){
  const [appts,setAppts]=useState(isDemo?INIT_APPTS:[]);
  const [waitlist,setWaitlist]=useState(isDemo?WAITLIST_INIT:[]);
  const [sel,setSel]=useState(null);
  ...
  // Load from Parse on mount (skip if demo)
  useEffect(()=>{
    if(isDemo){ setDbLoading(false); return; }
    let cancelled=false;
    (async()=>{
      try{
        ...
        if(!cancelled){ setAppts(a); setWaitlist(w); }
      } catch(e){
        if(!cancelled){ setAppts([]); setWaitlist([]); }
      } finally {
    ...
    <div className="screen screen-enter">
      ...
      <div className="sec">
        {(tab==="today"||tab==="upcoming")&&<>
          <p className="stit">{`Today · ${appts.length} appts`}</p>
          {appts.length===0&&!dbLoading&&<div style={{textAlign:"center",padding:"40px 20px",color:C.muted}}>
            <p style={{fontSize:40,marginBottom:12}}>📅</p>
            <p style={{fontSize:16,fontWeight:600,color:C.text,marginBottom:6}}>No appointments yet</p>
            <p style={{fontSize:13,marginBottom:20}}>Book your first client to get started</p>
            <button className="btn bg" onClick={onAdd} style={{width:"auto",padding:"10px 24px"}}>+ Add Appointment</button>
          </div>}
          {appts.map(a=>(
...
// ══════════════════════════════════════════
function Payments({isDemo=false}){
  ...
  const PAY_TXS=isDemo?TXS:[];
  const mTotal=PAY_TXS.filter(t=>t.type==="in").reduce((s,t)=>s+t.amount+t.tip,0);
  ...
  return(
    <div className="screen screen-enter">
      ...
      {tab==="transactions"&&<div className="sec">
        {PAY_TXS.length===0&&<div style={{textAlign:"center",padding:"40px 20px",color:C.muted}}><p style={{fontSize:40,marginBottom:12}}>💳</p><p style={{fontSize:16,fontWeight:600,marginBottom:6}}>No transactions yet</p><p style={{fontSize:13}}>Your payment history will appear here</p></div>}
        {PAY_TXS.map(tx=>(
...
export default function App(){
  ...
  const handleAuth=async(role, mode)=>{
    try{
      // Re-fetch user from server to get latest fields (shopName, etc.)
      const current=ParseService.currentUser();
      if(current) await current.fetch();
    } catch(e){}
    // Demo mode — skip Parse entirely, show mock data
    if(mode==="demo"){
      setIsDemo(true);
      setUserData({shopName:"Fresh Cutz",barberName:"Marcus Johnson",role:"barber"});
      setAppState("main");
      return;
    }
    setIsDemo(false);
  ...
  return(
    <>
      ...
      <div className="app">
        ...
        {/* Screens */}
        {tab==="home"&&<Dashboard onAdd={()=>setAddOpen(true)} onAdmin={()=>setAdminOpen(true)} onBookingPage={()=>setBookingOpen(true)} onWaitlist={()=>setWaitlistOpen(true)} onNoShow={()=>setNoShowOpen(true)} shopName={userData.shopName} isDemo={isDemo}/>}
        {tab==="appts"&&<Appointments onAdd={()=>setAddOpen(true)} onWaitlist={()=>setWaitlistOpen(true)} onNoShow={()=>setNoShowOpen(true)} isDemo={isDemo}/>}
        {tab==="clients"&&<ClientsScreen isDemo={isDemo}/>}
        {tab==="pay"&&<Payments isDemo={isDemo}/>}
        {tab==="more"&&!moreTab&&(
          <div className="screen screen-enter">
        ...
        {tab==="more"&&moreTab==="profile"&&<Profile onAdmin={()=>setAdminOpen(true)} onBack={()=>setMoreTab(null)} onSignOut={async()=>{try{await ParseService.logOut();}catch(e){}setIsDemo(false);setAppState("auth");setTab("home");setMoreTab(null);}} userData={userData}/>}
